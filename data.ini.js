dataIniLines = function(){/*--keep this line--
[*]
...
; === Added 16 March 2023 ===
export-srt          = SRT format (.srt)
export-none-time    = Plain text without time(.txt)
...
more-backups        = 載入更多備份
; === Added 27 May 2015 ===
backup-clear        = 您的瀏覽器本地存儲空間已滿，因此一些較舊的 oTranscribe 備份已被清除。
backup-error        = oTranscribe在保存時遇到問題。請確保手動備份您的工作。
restore-error       = 無法還原備份。
; === Added 3 January 2017 ===
jump-to-time        = 跳轉到時間:
; === Added 2 May 2017 ===
settings            = 設置
...
underline           = 底線字
addTimestamp        = 插入時間戳記
returnToStart       = 回轉到最前面
; === Added 16 March 2023 ===
export-srt          = SRT 字幕格式 (.srt)
export-none-time    = 去掉時間的文字稿(.txt)
privacy             = 隱私政策
; === No longer used ===
; history-instrux
[da]
...
[ru]
help	= Справка
lead	= Бесплатное веб-приложение для облегчения расшифровки аудиозаписей интервью.
start-loading	= Загрузка...
start-ready	= Начать расшифровку
mobile-warning	= oTranscribe работает только на настольных компьютерах.
old-browser-warning	= Ваш браузер не поддерживает некоторые возможности HTML5, необходимые для корректной работы oTranscribe. Используйте современную версию браузера для настольных компьютеров.
promo-1.innerHTML	= Больше не нужно переключаться между Quicktime и Word.
promo-2.innerHTML	= Приостановка, перемотка назад и вперед без отрыва от клавиатуры.
promo-3.innerHTML	= Интерактивные временные метки для быстрого перехода по расшифровке.
promo-4.innerHTML	= Постоянное автоматическое сохранение в локальном хранилище браузера.
promo-5.innerHTML	= Конфиденциально — аудиофайл и его расшифровка всегда находятся на вашем компьютере.
promo-6.innerHTML	= Экспорт в Markdown, обычный текст и Google Документы.
promo-7.innerHTML	= Поддержка видеофайлов во встроенном видеопроигрывателе.
promo-8.innerHTML	= Открытый исходный код по лицензии MIT.
press	= oTranscribe в СМИ
twitter	= Следить за @oTranscribe (на английском)
credit	= Автор
choose-file	= Выберите аудио (или видео) файл
formats	= Ваш браузер поддерживает [xxx] аудиофайлы и [yyy] видеофайлы. Возможно вам потребуется конвертировать файлы.
format-warn	= Ваш браузер не поддерживает [file-format] файлы. Используйте другой браузер или конвертируйте файл в другой формат...
wordcount	= слов: {{n}}
export	= Экспорт
export-download	= Скачать расшифровку как...
export-markdown	= Markdown (.md)
export-text	= Обычный текст (.txt)
export-send	= Отправить расшифровку...
sign-in	= Войти
send-drive	= Отправка на Диск...
open-drive	= Открыть на Диске
file-name	= Расшифровка сохранена
speed	= скорость
last-file	= Последний файл:
d-txt1.innerHTML	= Введите текст расшифровки...
d-txt2.innerHTML	= Краткие советы:
d-txt3.innerHTML	= - Чтобы выделить текст курсивом, нажмите клавиши Ctrl+I. Для полужирного начертания нажмите клавиши Ctrl+B.
d-txt4.innerHTML	= - Нажмите ESC для воспроизведение и остановки, нажмите клавиши Ctrl+J для вставки текущей временной метки.
export-otr	= формат oTranscribe
choose-youtube	= или YouTube видео
youtube-instrux	= Введите URL-адрес YouTube видео
youtube-error	= Введите допустимый URL-адрес YouTube Например: https://www.youtube.com/watch?v=dQw4w9WgXcQ
history-button	= история
import-button	= импорт
history-title	= История Transcript
history-instrux-v2	= Сохранение вашей работы происходит раз в пять минут. Сохраняются только 100 резервных копий. Чтобы сохранить свою работу в любое время, нажмите клавиши Ctrl+S
today	= Сегодня
yesterday	= Вчера
restore-button	= восстановить
no-backups	= Резервных копий не найдено.
more-backups	= Загрузить старые резервные копии
backup-clear	= Локальное хранилище вашего браузера было переполнено, поэтому некоторые старые резервные копии oTranscribe были удалены.
backup-error	= Произошла ошибка при сохранении. Создайте резервную копию вручную.
restore-error	= Невозможно восстановить резервную копию.
[es]
...
promo-1.innerHTML   = <strong>No más enredos</strong> con Quicktime y Word.
promo-2.innerHTML   = <strong>Pausa, rebobina o adelanta</strong> tu grabación sin alejarte del teclado.
promo-3.innerHTML   = <strong>Etiquetas interactivas de tiempo </strong> para navegar por tu transcripción.
promo-4.innerHTML   = <strong>Guarda tu trabajo automáticamente</strong> cada segundo en el almacenamiento de tu navegador.
promo-5.innerHTML   = <strong>Privacidad</strong>: tus grabaciones y documentos no salen de tu ordenador.
promo-6.innerHTML   = <strong>Exporta tu trabajo</strong> a Markdown (.md), texto plano (.txt) y Google Drive.
promo-7.innerHTML   = <strong>Vídeo compatible</strong> con el reproductor HTML5 integrado.
promo-8.innerHTML   = <strong><a href="opensource">Aplicación de código abierto</a></strong> (en inglés) bajo licencia <a href="http://es.wikipedia.org/wiki/MIT_License">MIT</a>.
...
formats             = Tu navegador es compatible con archivos de audio en formato [xxx] o de video en formato [yyy]. Tal vez necesites <a href="http://media.io/es">convertir tu archivo</a>.
format-warn         = Tu navegador no es compatible con archivos en formato [file-format]. Intenta con otro navegador o <a href="http://media.io/es">convierte tu archivo</a> en otro formato.
...
d-txt3.innerHTML    = - <em>Ctrl+I</em> añade <em>italicas</em> y <b>Ctrl+B</b> añade <b>negritas</b>.
...
help                = Hjälp
; === homepage ===
lead                = En kostnadsfri webbapp för enklare transkribering av inspelade intervjuer.
start-loading       = Laddar...
start-ready         = Börja transkribera
mobile-warning      = oTranscribe fungerar endast på fullvärdiga datorer.
old-browser-warning = Din webbläsare verkar inte stödja vissa HTML5 funktioner som behövs för att oTranscribe ska fungera korrekt. Vänligen byta till en modern webbläsare.
promo-1.innerHTML   = <strong>Inget mer växlande</strong> mellan Quicktime och Word.
promo-2.innerHTML   = <strong>Pausa, spola och ändra hastigheten</strong> utan att lämna tangentbordet.
promo-3.innerHTML   = <strong>Interaktiva tidsstämplar</strong> för att navigera genom din transkribering.
promo-4.innerHTML   = <strong>Sparar automatiskt</strong> till din webbläsare varje sekund.
promo-5.innerHTML   = <strong>Privat</strong> - din ljudfil och transkribering lämnar aldrig din dator.
promo-6.innerHTML   = <strong>Exportera</strong> till Markdown, råtext och Google Docs.
promo-7.innerHTML   = <strong>Stöd för video filer</strong> med integrerad spelare.
promo-8.innerHTML   = <strong><a href="opensource">Open source</a></strong> under MIT licensen.
press               = Känt från
twitter             = Följ @oTranscribe
credit              = Skapad av
; === editor ===
speed               = uppspelningshastighet
choose-file         = Välj ljud- (eller video) fil
last-file           = Senast använd fil:
formats             = Din webbläsare stödjer [xxx] ljudfiler och [yyy] videofiler. Du kan behöva <a href="http://media.io">konvertera din fil</a>.
format-warn         = Din webbläsare stöder inte [fil-format] filer. Byt till en annan webbläsare eller <a href="http://media.io">konvertera filen</a> till ett annat format.
wordcount           = ord
export              = Exportera
export-download     = Ladda ner transkribering som…
export-markdown     = Markdown (.md)
export-text         = Råtext (.txt)
export-send         = Skicka transkribering till...
sign-in             = Logga in
send-drive          = Skickar till Drive...
open-drive          = Öppna i Drive
file-name           = Transkribering exporterad
d-txt1.innerHTML    = Skriv din transkribering här...
d-txt2.innerHTML    = Snabbtips:
d-txt3.innerHTML    = - <em>Ctrl+I</em> växlar till <em>kursiv</em> stil och <b>Ctrl+B</b> växlar till <b>fet</b> stil.
d-txt4.innerHTML    = - Tryck ESC för uppspelning/paus, och Ctrl+J för att lägga till aktuell tidsstämpel.
; === Added 26 Jan 2014 ===
export-otr          = oTranscribe format
choose-youtube      = eller YouTube video
youtube-instrux     = Ange YouTube video URL:
youtube-error       = Vänligen ange giltig YouTube URL. Till exampel: https://www.youtube.com/watch?v=dQw4w9WgXcQ
history-button      = historik
import-button       = importera
history-title       = Transkriberings historik
history-instrux-v2  = En kopia av ditt arbete sparas var femte minut. Säkerhetskopior sparas inte särskilt länge. Tryck Ctrl+S för att spara när som helst.
today               = Idag
yesterday           = Igår
restore-button      = återställ
no-backups          = Inga säkerhetskopior funna.
more-backups        = Ladda äldre säkerhetskopior
; === Added 27 May 2015 ===
backup-clear        = Ditt lokala webbläsarlagringsutrymme var fullt, så några äldre oTranscribe säkerhetskopior har rensats.
backup-error        = oTrancribe har stött på ett problem under nedsparandet. Vänligen säkerhetskopiera ditt arbete manuellt.
restore-error       = Säkerhetskopian kunde inte återställas.
[tr]
; The context of these messages is available here: https://github.com/oTranscribe/oTranscribe/wiki/Help-translate-oTranscribe#message-contexts
help                = Yardım
; === anasayfa ===
lead                = Kayıtlı röportajlarınızı deşifre etmek için çektiğiniz zahmeti kaldıran, ücretsiz bir web uygulaması.
start-loading       = Yükleniyor...
start-ready         = Deşifreye başlayın
mobile-warning      = oTranscribe sadece masaüstü bilgisayarlarda çalışır.
old-browser-warning = Browser'ınız, oTranscribe'ın doğru çalışması için gerekli olan bazı HTML5 özelliklerini desteklemiyor. Lütfen modern bir masaüstü browser'a geçin.
promo-1.innerHTML   = Video oynatıcı ile Word arasında <strong>bir ona bir buna geçip durmak yok</strong> .
promo-2.innerHTML   = Elinizi klavyeden kaldırmadan <strong>durdurun, geri alın, ileri alın</strong> .
promo-3.innerHTML   = <strong>İnteraktif zaman kaydı</strong> ile metin üzerinden ses kaydında gezinebilirsiniz.
promo-4.innerHTML   = Her saniye browser'ınıza ayrılan diske <strong>otomatik olarak yedeklenir</strong>.
promo-5.innerHTML   = <strong>Gizliliğinize riayet eder </strong>, ses dosyanız asla bilgisayarınızdan başka bir yere aktarılmaz.
promo-6.innerHTML   = Markdown, düz metin yahut Google Dökümanı olarak <strong>dışa aktarın</strong>.
promo-7.innerHTML   = Entegre oynatıcı ile <strong>video dosyalarınızı da destekler</strong>.
promo-8.innerHTML   = MIT lisansı ile <strong><a href="opensource">açık kaynak kodludur</a></strong>.
press               = Hakkımızda basında çıkan haberler
twitter             = @oTranscribe'ı twitterda takip et
credit              = Hazırlayan
; === editor ===
speed               = hız
choose-file         = Ses (veya video) dosyası seçin
last-file           = Son açılan dosya:
formats             = Browser'ınız [xxx] ses dosyalarını ve [yyy] video dosyalarını destekliyor. Dosyanızı <a href="http://media.io">dönüştürmeniz</a> gerekibilir.
format-warn         = Browser'ınız [file-format] dosyalarını desteklemiyor. Başka bir browser'a geçin veya  dosyayı <a href="http://media.io">başka bir formata dönüştürün</a>.
wordcount           = {{n}} kelime
export              = Dışa Aktar
export-download     = Metni farklı kaydet
export-markdown     = Markdown (.md)
export-text         = Düz Metin (.txt)
export-send         = Metni gönder...
sign-in             = Giriş Yap
send-drive          = Sürücüye gönderiliyor...
open-drive          = Sürücüde aç
file-name           = Metin dışa aktarıldı
d-txt1.innerHTML    = Metninizi buraya yazın...
d-txt2.innerHTML    = İpuçları:
d-txt3.innerHTML    = - <em>Ctrl+I</em>  metni <em>italik</em> yapar, <b>Ctrl+B</b> metni <b>koyu</b> yapar.
d-txt4.innerHTML    = - Oynatmak/durdurmak için ESC'ye basın, şu anki zaman kaydı girmek için Ctrl+J'ye basın.
; === Added 26 Jan 2014 ===
export-otr          = oTranscribe formatı
choose-youtube      = YouTube videosu seçin
youtube-instrux     = YouTube videosunun adresini girin:
youtube-error       = Lütfen geçerli bir YouTube adresi girin. Örneğin: https://www.youtube.com/watch?v=dQw4w9WgXcQ
history-button      = geçmiş
import-button       = içe aktar
history-title       = Deşifre geçmişi
history-instrux-v2  = Her beş dakikada bir, çalışmanızın bir kopyası kaydedilmektedir. Toplamda yalnız 100 yedek tutulmaktadır. Kaydetmek istediğinizde Ctrl+S ye basın.
today               = Bugün
yesterday           = Dün
restore-button      = geri yükle
no-backups          = Yedek bulunamadı.
more-backups        = Daha eski bir yedeği yükle
; === Added 27 May 2015 ===
backup-clear        = Browserınızın yerel depolama alanı dolu, bu yüzden bazı eski oTranscribe yedekleri silindi.
backup-error        = oTranscribe kaydederken bir problemle karşılaştı. Lütfen çalışmanızı kendiniz elle yedekleyin.
restore-error       = Yedek geri yüklenemedi.
; === No longer used ===
; history-instrux
[uk]
; The context of these messages is available here: https://github.com/oTranscribe/oTranscribe/wiki/Help-translate-oTranscribe#message-contexts
...
promo-1.innerHTML   = <strong>Без перемикань</strong> між Quicktime та Word.
promo-2.innerHTML   = <strong>Пауза, і перемотки назад та вперед</strong> не забираючи руки з клавіатури.
promo-3.innerHTML   = <strong>Інтерактивні мітки часу</strong> щоб переміщатися по розшифровці.
promo-4.innerHTML   = <strong>Автоматично записується</strong> до сховища вашого переглядача кожної секунди.
promo-5.innerHTML   = <strong>Приватно</strong> - ваш аудіо файл та розшифровка ніколи не покидає Вашого комп'ютера.
promo-6.innerHTML   = <strong>Експортуйте</strong> в Markdown, прості текстові файли та Google Docs.
promo-7.innerHTML   = <strong>Підтримка Відео файлів</strong> з вбудованим програвачем.
promo-8.innerHTML   = <strong><a href="opensource">Відкритий код</a></strong> під ліцензією MIT.
press               = В пресі:
...
formats             = Ваш переглядач підтримує наступні аудіо файли: [xxx] та наступні відео файли: [yyy]. Можливо вам потрібно <a href="http://media.io">перетворити ваш файл</a>.
format-warn         = Ваш переглядах не підтримує файли формату [file-format]. Перемкніться на інший переглядач чи <a href="http://media.io">перетворіть ваш файл</a> в інший формат.
wordcount           = {{n}} слова
export              = Експорт
...
d-txt3.innerHTML    = - <em>Ctrl+I</em> додає <em>курсив</em> а <b>Ctrl+B</b> додає <b>жирне</b> форматування.
...
help                = Trợ giúp
; === homepage ===
lead                = Một ứng dụng web miễn phí để ghi chép lại các bài phỏng vấn đã ghi âm.
start-loading       = Đang tải...
start-ready         = Bắt đầu việc chép lại
mobile-warning      = oTranscribe chỉ làm việc trên môi trường máy tính để bàn mà thôi.
old-browser-warning = Trình duyệt của bạn dường như không hỗ trợ một số tính năng HTML5 thiết yếu để cho oTranscribe có thể chạy một cách đúng đắn. Hãy chuyển sang dùng trình duyệt hiện đại hơn.
promo-1.innerHTML   = <strong>Không cần hoán chuyển</strong> giữa Quicktime và Word.
promo-2.innerHTML   = <strong>Tạm dừng, tua và chuyển-tiếp-nhanh</strong> mà không cần đưa bàn tay của bạn khỏi bàn phím.
promo-3.innerHTML   = <strong>Đánh dấu thời gian tương tác</strong> để dùng nó điều hướng toàn bộ bản chép lại của bạn.
promo-4.innerHTML   = <strong>Lưu tự động</strong> vào phần kho lưu trữ của trình duyệt của bạn mỗi giây.
promo-5.innerHTML   = <strong>Riêng tư</strong> - tập tin âm thanh và bản chép lại không bao giờ rời khỏi máy tính của bạn.
promo-6.innerHTML   = <strong>Xuất</strong> sang Markdown, văn bản thường và Google Docs.
promo-7.innerHTML   = <strong>Hỗ trợ các tập tin phim</strong> với bộ phát tích hợp.
promo-8.innerHTML   = <strong><a href="opensource">Nguồn mở</a></strong> dưới giấy phép MIT.
press               = Được đánh giá cao bởi
twitter             = Theo dõi @oTranscribe
credit              = Được tạo bởi
; === editor ===
speed               = tốc độ
choose-file         = Chọn tập tin âm thanh (hay phim)
last-file           = Tập tin dùng lần cuối:
formats             = Trình duyệt của bạn hỗ trợ các tập tin âm thanh [xxx] và phim [yyy]. Có lẽ bạn cần <a href="http://media.io">chuyển đổi tập tin của bạn</a>.
format-warn         = Trình duyệt của bạn không hỗ trợ các tập tin [file-format]. Hãy chuyển sang dùng trình duyệt khác hoặc <a href="http://media.io">chuyển đổi tập tin của bạn</a> sang định dạng khác.
wordcount           = {{n}} từ
export              = Xuất ra
export-download     = Tải bản chép lại về ở dạng...
export-markdown     = Markdown (.md)
export-text         = Văn bản thường (.txt)
export-send         = Gửi bản chép lại đến...
sign-in             = Đăng nhập
send-drive          = Gửi đến Drive...
open-drive          = Mở trong Drive
file-name           = Bản ghi lại xuất vào
d-txt1.innerHTML    = Nhập phần chép lại của bạn tại đây...
d-txt2.innerHTML    = Mẹo nhỏ:
d-txt3.innerHTML    = - <em>Ctrl+I</em> thêm ở dạng chữ <em>nghiêng</em> và <b>Ctrl+B</b> ở dạng chữ <b>đậm</b>.
d-txt4.innerHTML    = - Nhấn phím ESC để phát/dừng, và Ctrl+J để chèn thời gian hiện tại.
; === Added 26 Jan 2014 ===
export-otr          = Định dạng oTranscribe
choose-youtube      = hay phim YouTube
youtube-instrux     = Nhập vào địa chỉ URL của YouTube:
youtube-error       = Vui lòng nhập vào địa chỉ URL của YouTube. Ví dụ như là: https://www.youtube.com/watch?v=dQw4w9WgXcQ
history-button      = lịch sử
import-button       = nhập vào
history-title       = Lịch sử các bản chép lại
history-instrux-v2  = Công việc của bạn sẽ được sao lưu lại năm phút một lần. Chỉ giữ lại 100 bản sao. Nhấn Ctrl+S để lưu bất kỳ lúc nào bạn muốn.
today               = Hôm nay
yesterday           = Hôm qua
restore-button      = phục hồi lại
no-backups          = Không tìm thấy bản sao dự phòng nào.
more-backups        = Tải bản sao dự phòng cũ hơn
; === Added 27 May 2015 ===
backup-clear        = Phần lưu trữ của trình duyệt của bạn đã bị đầy, so vậy các bản sao lưu dự phòng oTranscribe quá cũ hơn sẽ bị xóa bỏ.
backup-error        = oTranscribe gặp lỗi khi đang ghi dữ liệu lại. Vui lòng chắc chắn rằng bạn đã sao lưu lại công việc của mình bằng tay.
restore-error       = Không thể phục hồi lại bản sao dự phòng.
; === No longer used ===
; history-instrux

-----*/}.toString().replace(/\r/g,"").slice("function(){/*--keep this line--".length+1,-9);
